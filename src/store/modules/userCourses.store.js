...
const actions = {
  async GET_USER_COURSES({ getters, commit }, payload) {
    commit('LOADING', true);
    commit('ERROR', null);
    const { onlineCourses, error } = await (await fetch(`${getters.userCoursesEndpoint}?userId=${payload}`)).json();
    if (!error) {
  ...
  async GET_USER_COURSE_ID({ getters, commit }, payload) {
    const { onlineCourse } = await (await fetch(`${getters.userCoursesEndpoint}/${payload}`)).json();
    commit('CURRENT_COURSE', onlineCourse);
    commit('CURRENT_COURSE_VIDEOS', onlineCourse.videos);
    commit('CURRENT_COURSE_ID', payload);
  },
  async PUT_USER_COURSE_ID({ getters, commit, dispatch }, { data, id }) {
    const { error } = await (
      await fetch(`${getters.userCoursesEndpoint}/${id}`, {
        method: 'PUT',
        body: data,
      })
    ).json();
  ...
  async PUT_CURRENT_VIDEO({ getters, dispatch }, { fd, id }) {
    const {
      error,
      // updatedonline
    } = await (
      await fetch(`${getters.userCreateVideosCourse}/${id}`, {
        method: 'PUT',
        body: fd,
      })
    ).json();
  ...
  async CREATE_VIDEOS_COURSE({ getters, commit, dispatch }, payload) {
    commit('LOADING', true);
    commit('ERROR', null);
    commit('UPLOADING', true);
    const { error } = await (
      await fetch(`${getters.userCreateVideosCourse}/${payload.id}`, {
        method: 'POST',
        body: payload.fd,
      })
    ).json();
  ...
  async GET_VIDEO_COURSE_ID({ getters, commit }, id) {
  ...
  async CREATE_ONLINE_COURSE({ getters, commit, dispatch }, payload) {
    commit('LOADING', true);
    commit('ERROR', null);
    const { newOnlineCourse, error } = await (
      await fetch(getters.userCreateOnlineCourseEndpoint, {
        method: 'POST',
        body: payload.fd,
      })
    ).json();
  ...
  async REMOVE_COURSE({ getters, dispatch }, { courseId, userId }) {
    const response = await (
      await fetch(`${getters.removeCourseEndpoint}/${courseId}`, {
        method: 'DELETE',
      })
    ).json();
    if (!response.error) {
  ...
  async REMOVE_VIDEO_COURSE({ getters, dispatch }, { id, courseId }) {
    const { error } = await (
      await fetch(`${getters.userCreateVideosCourse}/${id}`, {
        method: 'DELETE',
      })
  ...
  async ADD_PDF({ getters, dispatch }, { fd, videoId, userId, currentCourseId }) {
    const { error } = await (
      await fetch(`${getters.addPdfEndpoint}/${videoId}`, {
        method: 'POST',
        body: fd,
      })
    ).json();
  ...
  async REMOVE_PDF({ getters, dispatch, commit }, { id, userId, videoId, currentCourseId }) {
    commit('ERROR', null);
    const { error } = await (
