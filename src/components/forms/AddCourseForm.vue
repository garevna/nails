...
const schema = require('@/config/addCourseOnlineSchema').default
...
export default {
  ...
  data () {
    return {
      schema,
      data: Object.keys(
        Object.assign({}, schema.sideLeft, schema.sideRight)
      ).reduce((acc, key) => {
        const obj = { [key]: '' }
        if (
        ...
        if (
          schema.sideLeft[key]?.type === 'file' ||
          schema.sideRight[key]?.type === 'file'
        ) {
          obj[key] = null
        }
